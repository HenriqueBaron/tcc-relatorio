...
\begin{document}
	...
	\begin{figure}[h]
		\incluirimagem[0.5]{ModeloImpulsoUnicoTri.png}{Perfil de impulso triangular de impacto}{o autor (\thedate)}
		\label{fig:modelo-imp-tri}
	\end{figure}

	\begin{figure}[h]
		\incluirimagem[0.5]{ModeloImpulsoUnicoRect.png}{Perfil de impulso retangular de impacto}{o autor (\thedate)}
	...
	Cada perfil de impulso foi testado com dois algoritmos diferentes para a solução do sistema de equações diferenciais da Equação \ref{eqn:mdl:sistema}:
	\begin{enumerate}
	...
	Após avaliar a combinação mais apropriada de perfil de impulso e mecanismo de solução, uma simulação com duração de quatro segundos é realizada para obter a resposta de vibração ao longo do tempo e o seu espectro de frequências.
	...
	Um deles foi o aparecimento de componentes imaginárias na carga máxima \gls{s:rol-carga-maxima-elemento}, e o outro foi a incapacidade de convergência do modelo devido à natureza impulsiva da força externa trazida pela função \gls{s:fun-forca-impacto} quando modelada com impulsos unitários como ilustrado na \figurename\ \ref{fig:modelo-imp-kron}.
	...
	Dos três perfis de impulso para a força de impacto testados, apenas dois levaram o modelo a convergir.
	Com ambos os algoritmos para a solução das equações diferenciais, descritos no item \ref{sec:mdl:sol-sistema}, a utilização de impulsos unitários com o delta de Kronecker como exibido na \figurename\ \ref{fig:modelo-imp-kron} impedia o cálculo de uma solução.
	A investigação da causa vai além dos objetivos deste trabalho.
	Portanto, apenas as formas de impulso triangular (\figurename\ \ref{fig:modelo-imp-tri}) e retangular (\figurename\ \ref{fig:modelo-imp-rect}) foram consideradas.
